...
def download_model():
    ...
    mod = BertForSequenceClassification(
        ...
        num_labels=cfg['mod']['num_classes'],  # 5
        dropout=cfg['mod']['dropout'],  # 0.25
    )
    mod.load_state_dict(torch.load("ml_model/DeepPavlov-Ru-bert-fine-tuned.pt", map_location=torch.device('cpu')))
